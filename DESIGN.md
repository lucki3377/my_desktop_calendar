...
### 4.13 표시 옵션 (2026-08-17 추가)

위젯 우클릭 메뉴의 체크 항목으로 켜고 끈다. 둘 다 기본값은 꺼짐.

- **월요일 시작**(`Widget.WeekStartsOnMonday`): 요일 헤더와 날짜 배치를 월요일 시작으로 바꾼다. 주말 색(토=파랑, 일=빨강)은 위치가 아니라 요일을 보고 칠하므로 그대로 유지된다
- **음력 표시**(`Widget.ShowLunarDates`): 날짜 숫자 **바로 옆에** 작게 음력을 붙인다(`KoreanLunarDate`). 칸 오른쪽 끝에 두면 옆 칸 숫자에 붙어 보여서 숫자 옆으로 옮겼다. 윤달은 "윤6.15"처럼 표기
- 헤더에 **"오늘"** 버튼 추가 — 몇 달 넘겨본 뒤 이번 달로 한 번에 돌아온다
- 체크 항목은 `Click`이 아니라 `Checked`/`Unchecked`에 반응한다. `Click`만 쓰면 체크 상태가 프로그램적으로 바뀔 때 반영되지 않는다

### 4.14 음력으로 일정 등록 (2026-08-17 추가)

음력 생일·제사처럼 음력으로 날짜를 아는 일정을 위한 기능. 두 부분으로 나뉜다.

- **음력으로 날짜 고르기**: 일정 편집 창의 "음력으로..." 버튼 → `LunarDatePickerDialog`에서 음력 년/월/일(+윤달)을 고르면 양력으로 환산해 시작 날짜에 넣는다. 고르는 즉시 환산 결과를 보여주고, 그 해에 없는 날짜(없는 윤달, 29일까지인 달의 30일)면 확인 버튼이 잠긴다. 시작 날짜 아래에는 늘 "음력 8.15"처럼 현재 날짜의 음력을 표시한다
- **음력 매년 반복**(`RecurrenceType.LunarYearly`): 해마다 같은 **음력** 날짜에 열린다 → 양력 날짜는 매년 달라진다. 예: 음력 8월 15일로 잡으면 2026년 9/25, 2027년 9/15에 열린다
- 저장은 어디까지나 **양력 시작 날짜 한 개**다. 반복을 펼칠 때 그 날짜를 음력으로 되바꿔 기준 음력 월/일을 얻는다 — 음력 정보를 따로 저장하지 않으므로 열이 늘지 않는다
- 윤달에 잡힌 일정인데 그 해에 같은 윤달이 없으면 **평달로 지낸다**(제사 관행과 같은 처리). 그 달에 그 날짜 자체가 없으면 그 해는 건너뛴다
- `.ics`로 내보낼 때 음력 반복은 RRULE로 표현할 수 없으므로 앞으로 20년치를 `RDATE`로 나열한다

### 4.15 설정 창 / 자동 실행 (2026-08-17 추가)

- **설정 창**(`SettingsWindow`): 우클릭 메뉴에 항목이 11개까지 늘어나 흩어져 있던 것을 한 곳으로 모았다. 그룹은 표시 / 연동 / 데이터 / 시작
  - **확인·취소가 없다.** 값을 바꾸면 곧바로 저장하고 `SettingsChanged` 이벤트로 위젯에 알린다 → 창을 열어 둔 채로 결과를 보며 조절할 수 있다
  - 위젯은 `ReloadDisplaySettings()` 하나로 저장소에서 다시 읽어 반영한다. 설정 항목마다 적용 코드를 따로 두지 않는다
  - 각 행에 현재 상태를 함께 보여준다(연결된 구글 계정과 캘린더 수, API 키 유무, D-day 개수, 현재 색상/불투명도)
  - 우클릭 메뉴는 설정 / 위치 잠금 / D-day 관리 / 도움말 / 종료 로 줄였다. `FontSizeDialog`는 설정 창의 슬라이더로 대체되어 삭제
- **자동 실행**(`AutoStartService`): 레지스트리 Run 키가 아니라 **시작프로그램 폴더에 바로가기(.lnk)** 를 만든다 (DESIGN.md 4.9 방침 — 사용자가 눈으로 확인·삭제할 수 있고 작업 관리자 시작 프로그램 탭에도 보인다)
...
- Recurrence: enum(None, Daily, Weekly, Monthly, Yearly, LunarYearly)  (2026-08-17 추가)
...
### Phase 6 — 마무리 ✅ 완료 (2026-08-17)
- [x] 트레이 아이콘 + 컨텍스트 메뉴(달력 보이기·숨기기 / 설정 / 도움말 / 종료) — 2026-08-17
- [x] Windows 시작 시 자동 실행 등록/해제 (`AutoStartService`, 시작프로그램 폴더 바로가기)
- [x] 설정 창 UI 정리 (`SettingsWindow` — 표시/연동/데이터/시작 4개 그룹)
- [x] self-contained exe 빌드 (`dotnet publish -c Release -r win-x64 --self-contained`, 트리밍 없음) — 실행 확인 + Windows Defender 로컬 검사 통과. VirusTotal 업로드는 사용자 판단에 맡김(업로드 시 파일이 백신 업체들에 공유됨)
...
- **2026-08-17**: 자잘한 표시 옵션 추가 (추천 목록의 "그 외"). 설계는 4.13 참조.
  - 헤더에 "오늘" 버튼, 우클릭 메뉴에 "월요일 시작"·"음력 표시" 체크 항목.
  - `KoreanLunarDate`(Core.Calendar): 양력 → 음력 변환, 윤달 표기. 테스트 11개(설날·추석·부처님오신날 날짜로 교차 검증, 2025년 윤6월 확인).
  - 체크 메뉴 항목을 처음엔 `Click`으로 달았다가, 체크 상태만 바뀌고 화면에 반영되지 않아 `Checked`/`Unchecked`로 교체(초기화 때 중복 렌더링하지 않도록 값이 같으면 빠져나온다).
  - 음력 라벨을 처음엔 칸 오른쪽 끝에 뒀는데 옆 칸 날짜 숫자에 붙어 보여서 숫자 바로 옆으로 옮김.
  - **실행 검증**: 두 옵션을 켠 상태로 스크린샷 확인(월~일 헤더, 각 날짜 옆 음력, 토=파랑·일=빨강 유지). 검증 후 둘 다 기본값(꺼짐)으로 되돌림.
  - `dotnet test` 88개 통과(음력 11개 신규).
- **2026-08-17**: 사용자가 구글 OAuth 클라이언트와 data.go.kr API 키를 직접 발급해 앱에 입력 → **그동안 미검증이던 두 경로가 모두 실동작 확인됨**.
  - 구글: 계정 연결 완료, 캘린더 3개 선택(본인 캘린더 + 대한민국 공휴일 + 가져온 캘린더), 이벤트 13건이 캐시에 들어옴(7/7~9/26). 2027년 달로 넘겨도 자동 재동기화되어 구글 일정이 표시되는 것까지 확인.
  - 공휴일 API: 2026년 22건을 받아와 내장 계산본을 전부 덮어씀(Source가 모두 `Api`). 겹치는 항목의 날짜는 내장 계산 결과와 정확히 일치했다.
  - **API가 내장 계산보다 더 넓은 집합을 준다**: 6/3 전국동시지방선거(계산으로는 알 수 없는 임시공휴일), 5/1 노동절, 7/17 제헌절이 추가로 들어왔다. 설계 의도대로 API 우선이라 문제는 없지만, 키 없이 내장 계산만 쓸 때는 이 세 날이 빠진다는 차이가 있다.
- **2026-08-17**: 음력으로 일정 등록 기능 추가 (사용자 요청). 설계는 4.14 참조.
  - `KoreanLunarDate.ToSolar` 추가(음력→양력, 윤달 인덱스 보정, 없는 날짜는 null). 기존 `Convert`(양력→음력)와 왕복 일치하는지 2026년 365일 전부로 검증.
  - `RecurrenceType.LunarYearly` 추가 + `RecurrenceExpander`가 음력 기준으로 회차를 계산. 저장은 양력 시작 날짜 하나뿐이고, 펼칠 때 음력으로 되바꿔 기준 월/일을 얻는다.
  - `LunarDatePickerDialog`(음력 년/월/일 + 윤달, 실시간 양력 미리보기) + 편집 창의 "음력으로..." 버튼, 시작 날짜 아래 음력 표시.
  - `IcsExporter`: 음력 반복은 RRULE로 못 적으므로 20년치를 `RDATE`로 나열.
  - **테스트 중 발견**: 내가 쓴 테스트가 2027년 설날을 2/6으로 기대했는데 실제로는 2/7이었다(2/6은 음력 섣달그믐). 라이브러리와 검증된 2024~2026년 값으로 교차 확인한 뒤 테스트 기대값을 고쳤다 — 코드가 옳았다.
  - **실행 검증**: "음력으로..."에서 음력 8월 15일 선택 → 양력 2026-09-25(추석)로 환산되는 것 확인 → "매년 (음력)"으로 저장 → **2027년 9월 15일**(그 해 음력 8.15, 추석)에 자동으로 옮겨 표시되는 것을 스크린샷으로 확인. 테스트 일정은 삭제함.
  - `dotnet test` 105개 통과(음력 일정 16개 신규).
- **2026-08-17**: Phase 6 마무리 — 설정 창 통합, 자동 실행, 배포 빌드. 설계는 4.15 참조. **Phase 0~6 전체 완료.**
  - `SettingsWindow` 신설(표시/연동/데이터/시작). 우클릭 메뉴를 11개 → 5개로 줄이고, 트레이 메뉴의 "색상/투명도"도 "설정"으로 교체. `FontSizeDialog`는 설정 창 슬라이더로 대체되어 삭제.
  - `WidgetWindow`에 흩어져 있던 설정 적용 코드를 `ReloadDisplaySettings()` 하나로 모음. 메뉴 항목별 핸들러 5개(API 키/구글/글자크기/색상/백업)를 제거.
  - `AutoStartService` 신설 — 시작프로그램 폴더 바로가기 방식.
